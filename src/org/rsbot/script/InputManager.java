...
public class InputManager {
    ...
    private boolean isOnCanvas(final int x, final int y) {
        return (x > 0) && (x < bot.getCanvas().getGameWidth()) && (y > 0) && (y < bot.getCanvas().getGameHeight());
    }
    ...
    public int getX() {
        return getClient().getMouse().getClientX();
    }

    public int getY() {
        return getClient().getMouse().getClientY();
    }
    ...
    public void moveMouse(final int speed, final int x, final int y, final int randomX, final int randomY) {
        ...
        if (!isOnCanvas(thisX, thisY)) {
            switch (random(1, 5)) { // on which side of canvas should it enter
                case 1:
                    thisX = -1;
                    thisY = random(0, bot.getCanvas().getGameHeight());
                    break;
                case 2:
                    thisX = random(0, bot.getCanvas().getGameWidth());
                    thisY = bot.getCanvas().getGameHeight() + 1;
                    break;
                case 3:
                    thisX = bot.getCanvas().getGameWidth() + 1;
                    thisY = random(0, bot.getCanvas().getGameHeight());
                    break;
                case 4:
                    thisX = random(0, bot.getCanvas().getGameWidth());
