...
public class Lobby extends MethodProvider {
    ...
    private final static Point[] TAB_POINTS = new Point[]{new Point(152, 42),
            new Point(258, 42), new Point(358, 42), new Point(461, 42), new Point(563, 42),
            new Point(655, 42)};
    ...
    public String[] getAvailableWorlds(boolean includingFull) {
        ...
        for (int i = 0; i < methods.interfaces.getComponent(WORLD_SELECT_INTERFACE, WORLD_SELECT_INTERFACE_WORLD_NAME).getComponents().length; i++) {
            String amount = methods.interfaces.getComponent(WORLD_SELECT_INTERFACE, WORLD_SELECT_INTERFACE_AMOUNT_OF_PLAYERS).getComponents()[i].getText();
            String number = methods.interfaces.getComponent(WORLD_SELECT_INTERFACE, WORLD_SELECT_INTERFACE_WORLD_NAME).getComponents()[i].getText();
    ...
    public RSComponent getWorldComponent(int world) {
        for (int i = 0; i < methods.interfaces.getComponent(WORLD_SELECT_INTERFACE, WORLD_SELECT_INTERFACE_WORLD_NAME).getComponents().length; i++) {
            RSComponent comp = methods.interfaces.getComponent(WORLD_SELECT_INTERFACE, WORLD_SELECT_INTERFACE_WORLD_NAME).getComponents()[i];
            if (comp != null) {
