...
public class GrandExchange extends MethodProvider {
    ...
    public boolean checkSlotIsEmpty(int slot) {
        try {
            ...
            if (isOpen()) {
                if (methods.interfaces.getComponent(INTERFACE_GRAND_EXCHANGE_WINDOW, slotComponent).getComponent(10).containsText("Empty")) {
                    return true;
                } else {
                    return false;
                }
    ...
    public String checkSlot(int slot) {
        try {
            int slotComponent = GRAND_EXCHANGE_OFFER_BOXES[slot];
            if (isOpen()) {
                if (methods.interfaces.getComponent(INTERFACE_GRAND_EXCHANGE_WINDOW, slotComponent).getComponent(10).isValid()) {
                    return methods.interfaces.getComponent(INTERFACE_GRAND_EXCHANGE_WINDOW, slotComponent).getComponent(10).getText();
                } else {
                    return null;
                }
            } else {
        ...
        for (int i = 1; i <= 6; i++) {
            if (isOpen()) {
                if (checkSlotIsEmpty(i)) {
                    ...
                    String s = methods.interfaces.getComponent(INTERFACE_GRAND_EXCHANGE_WINDOW, slotComponent).getComponent(18).getText();
                    if (s.equals(name)) {
                        return i;
                    }
                }
    ...
    public boolean checkCompleted(int slot) {
        if (!checkSlotIsEmpty(slot)) {
            if (slot != 0) {
                int slotComponent = GRAND_EXCHANGE_OFFER_BOXES[slot];
                if (methods.interfaces.getComponent(INTERFACE_GRAND_EXCHANGE_WINDOW, slotComponent).containsAction("Abort Offer")) {
                    return false;
                } else {
                    return true;
                }
    ...
    public void getItem(int slot) {
        ...
        if (isOpen()) {
            int slotComponent = GRAND_EXCHANGE_OFFER_BOXES[slot];
            methods.interfaces.getComponent(INTERFACE_GRAND_EXCHANGE_WINDOW, slotComponent).containsAction("Veiw Offer");
            sleep(random(700, 1200));
            if (methods.interfaces.getComponent(INTERFACE_GRAND_EXCHANGE_WINDOW, GRAND_EXCHANGE_COLLECT_BOX_TWO).containsAction("Collect")) {
                methods.interfaces.getComponent(INTERFACE_GRAND_EXCHANGE_WINDOW, GRAND_EXCHANGE_COLLECT_BOX_TWO).doAction("Collect");
                sleep(random(400, 900));
            }
            if (methods.interfaces.getComponent(INTERFACE_GRAND_EXCHANGE_WINDOW, GRAND_EXCHANGE_COLLECT_BOX_ONE).containsAction("Collect")) {
                methods.interfaces.getComponent(INTERFACE_GRAND_EXCHANGE_WINDOW, GRAND_EXCHANGE_COLLECT_BOX_ONE).doAction("Collect");
                sleep(random(400, 900));
            }
        }
    }
