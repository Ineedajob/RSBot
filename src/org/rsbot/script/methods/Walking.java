...
public class Walking extends MethodProvider {
	...
	public boolean walkPathOnScreen(RSTile[] path, int maxDist) {
		...
		if (next != null) {
			...
			return os != null && methods.tiles.doAction(os, "alk");
		}
		return false;
	}
	...
	public boolean walkTileOnScreen(RSTile tileToWalk) {
		return methods.tiles.doAction(methods.calc.getTileOnScreen(tileToWalk), "alk");
	}
	...
	public RSTile nextTile(RSTile path[], int skipDist) {
		...
		if (closest >= 0 && closest < path.length - 1) {
			if (closest < path.length - 2 && methods.calc.distanceTo(path[closest + 2]) < skipDist) {
				return path[closest + 2];
			} else {
