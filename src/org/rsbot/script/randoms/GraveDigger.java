...
public class GraveDigger extends Random {
    ...
    public boolean atInventoryItem2(final int itemID, final String option) {
        if ((game.getCurrentTab() != Game.TAB_INVENTORY) && !interfaces.get(Bank.INTERFACE_BANK).isValid() && !interfaces.get(Store.INTERFACE_STORE).isValid()) {
            game.openTab(Game.TAB_INVENTORY);
        }
        final RSItem[] items = inventory.getItems();
        ...
        final Point t = inventory.getItem(idx).getComponent().getCenter();
        mouse.click(t, 5, 5, false);
        sleep(random(150, 350));
        return menu.doAction(option);
    }
    ...
    public int loop() {
        ...
        if (inventory.getCountExcept(GraveDigger.coffinIDs) > 23) {
            ...
            if (depo != null) {
                if (!calc.tileOnScreen(depo.getLocation())) {
                    walking.walkPathMM(walking.findPath(depo.getLocation()));
        ...
        if (getMyPlayer().isMoving()) {
            ...
            final int modelID = interfaces.get(GraveDigger.INTERFACE_READ_GRAVESTONE).
            					getComponent(GraveDigger.INTERFACE_READ_GRAVESTONE_MODEL).getComponentID();
    ...
    public boolean setCharacterInScreen(final RSCharacter ch) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (!calc.pointOnScreen(screenLocation)) {
                switch (i) {
                    case 0:
                        camera.turnToCharacter(ch);

                        sleep(random(200, 500));

                        break;
                    case 1:
                        walking.walkTileMM(walking.getClosestTileOnMap(walking.randomizeTile(ch.getLocation(), 2, 2)));
                ...
                switch (i) {
                    ...
                    case 1:
                        walking.walkTileMM(walking.getClosestTileOnMap(
                        		walking.randomizeTile(obj.getLocation(), 2, 2)));
