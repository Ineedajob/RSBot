...
public class Certer extends Random {
    ...
    public int loop() {
        ...
        if (readyToLeave) {
            int PORTAL_ID=11368;final RSObject portal = objects.getNearest(PORTAL_ID);
            if (portal != null) {
                ...
                if (calc.distanceTo(portal) < 4) {
                    ...
                    walking.walkTileMM(walking.randomizeTile(new RSTile(portalLocation.getX() - 1, portalLocation.getY()), 1, 1));
        ...
        if (certer != null) {
            if (calc.distanceTo(certer) < 4) {
                ...
                walking.walkTileMM(walking.randomizeTile(new RSTile(certerLocation.getX() + 2, certerLocation.getY()), 1, 1));
                return random(6000, 8000);
            }
        }
