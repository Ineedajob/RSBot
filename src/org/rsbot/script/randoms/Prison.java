...
public class Prison extends Random {
        ...
        switch (state) {
            case 0:
                ...
                if (!talkedtopete && pete != null
                        && !(interfaces.get(228).isValid()) && !interfaces.canContinue()) {
    ...
    public boolean atLever() {
        if (interfaces.get(273).getComponent(3).isValid()) {
            final Filter<RSModel> filter = RSModel.newVertexFilter(setItemIDs(interfaces.get(273).getComponent(3).getModelID()));
            balloonToPop = npcs.getNearest(new Filter<RSNPC>() {
                public boolean accept(RSNPC n) {
                    if (filter.accept(n.getModel())) {
                        return true;
                    }
                    return false;
                }
