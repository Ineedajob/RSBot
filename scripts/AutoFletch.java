...
public class AutoFletch extends Script implements MessageListener, PaintListener {
	...
	private static final Log[] LOGS = new Log[] {
			...
			new Log("Oak Log", 0, 1521),
			new Log("Willow Log", 0, 1519), 
			new Log("Maple Log", 0, 1517),
			new Log("Yew Log", 0, 1515), 
			new Log("Magic Log", 0, 1513) 
    };
		...
		case BANK:
			if (!bank.isOpen()) {
				...
				if (!inventory.contains(single_log.id)) {
					...
					for (int i = 0; i < 100 && !inventory.contains(single_log.id); i++)
						sleep(10);
					bank.close();
					for (int i = 0; i < 100 && bank.isOpen(); i++)
						sleep(10);
	...
	static class Gui extends javax.swing.JFrame {
		...
		private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {
			setVisible(false);
			single_log = hideOptMap.get(jComboBox1.getSelectedItem().toString());
			guiOn = false;
		}
